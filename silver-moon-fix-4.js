...
/* FIX GỐC: mọi con trực tiếp của header mặc định min-width:auto trong flexbox,
   nên dù cha có overflow:hidden, con vẫn ép kích thước theo nội dung của nó,
   đẩy/tràn cả hàng qua phải. Ép min-width:0 cho TẤT CẢ con để buộc chúng co lại,
   bất kể đó là phần tử gì (không cần biết đúng tên class). */
#chat .mes_reasoning_details[data-state] .mes_reasoning_header > * {
    min-width: 0 !important;
    max-width: 100% !important;
    overflow: hidden !important;
...
/* Theme "Whisper" (Moonlit Echoes) có thể set flex:0 0 auto hoặc width cố định (px) trên
   chính .mes_reasoning_summary/.mes_reasoning_details khiến khung cha không co theo container
   thật của tin nhắn. Ép lại toàn bộ chuỗi cha-con về width tự nhiên, tăng specificity bằng
   cách lặp selector gốc (id giả lập) để thắng thứ tự nạp CSS của theme (theme nạp sau ta). */
html body #chat .mes_reasoning_details[data-state="thinking"],
html body #chat .mes_reasoning_details[data-state="done"] {
    width: 100% !important;
    max-width: 100% !important;
    flex: 0 1 100% !important;
    box-sizing: border-box !important;
}
html body #chat .mes_reasoning_details[data-state] .mes_reasoning_summary,
html body #chat .mes_reasoning_details[data-state] .mes_reasoning_header,
html body #chat .mes_reasoning_details[data-state] .mes_reasoning_header_title {
    width: 100% !important;
    max-width: 100% !important;
    box-sizing: border-box !important;
}
...
/* Dự phòng: một số bản ST hiển thị "Thought for X seconds" bằng phần tử NẰM NGOÀI
   .mes_reasoning_header_title (anh em cùng cấp trong .mes_reasoning_header), nên các
   rule phía trên không bắt trúng. Quét rộng thêm theo text/thuộc tính hay gặp. */
#chat .mes_reasoning_details[data-state] .mes_reasoning_header > *:not(.mes_reasoning_header_title):not(.sm-keep),
#chat .mes_reasoning_details[data-state] [class*="reasoning"][class*="duration"],
#chat .mes_reasoning_details[data-state] [class*="reasoning"][class*="time"],
#chat .mes_reasoning_details[data-state] [class*="reasoning"][class*="status"] {
    font-size: 0 !important;
    display: none !important;
}
